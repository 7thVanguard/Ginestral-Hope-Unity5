	...
	public class GUIGHMainMenu : MonoBehaviour 
	{
		...
		public static GameObject blackSpace;
		...
		public static float alphaCounterBlackScreen = 0;
		...
		void Update()
		{
			if (Input.GetKeyUp(KeyCode.R) || reset)
			{
				...
				if (Global.player.playerObj.transform.parent != null)
					Global.player.playerObj.transform.parent = null;
				if (Application.loadedLevelName == "Cavernina")
					CaverninaButton();
				else if (Application.loadedLevelName == "Catacombs")
					CatacombsButton();
			}
		...
		public void CaverninaButton()
		{
			...
			Global.player.currentLife = Global.player.maxLife - 1;
		}
